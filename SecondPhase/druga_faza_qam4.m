...
qam4_tx = qam4_tx_I_filt+ 1j * qam4_tx_Q_filt; 

win = transpose(hann(length(qam4_tx)));
nspec_len = length(qam4_tx);
nspec = ((1:nspec_len) - nspec_len./2) / nspec_len;
% Iscrtavanje spektra signala na izlazu predajnika
figure(1);
plot(nspec, 20*log10(fftshift(abs(fft(qam4_tx.*win)))));
title('Spektar signala na izlazu 4-QAM predajnika ');
ylabel('|4-QAM-TX(F)|');
xlabel('F');
...
hold on;
plot(nspec, 20*log10(fftshift(abs(fft(qam4_rx_kvant.*win)))));
...
title('Konstelacioni dijagram 4-QAM');
xlabel('I');
ylabel('Q');
